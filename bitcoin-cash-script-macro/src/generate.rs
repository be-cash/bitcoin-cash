...
impl GenerateScript {
    ...
    fn run_script(&mut self, script: Result<ir::Script, Error>) -> Result<TokenStream, Error> {
        ...
        let (input_struct_enum, impl_ops, impl_types, impl_names) = if script.script_variants.len()
            == 0
        {
            (
                quote! {
            ...
            for (variant_name, variant_fields) in enum_variant_fields {
                ...
                let field_names_str = variant_fields.iter().map(|(ident, _, _)| ident.to_string());
