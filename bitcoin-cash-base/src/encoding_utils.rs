...
fn try_shl(a: InnerInteger, b: u32, vec: &[u8]) -> Result<InnerInteger, String> {
    a.checked_shl(b)
        .ok_or_else(|| format!("Overflow for {}", hex::encode(vec)))
}

pub fn vec_to_int(vec: &[u8]) -> Result<InnerInteger, String> {
    if vec.is_empty() {
        return Ok(0);
    }
    let mut shift = 0;
