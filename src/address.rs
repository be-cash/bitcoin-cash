...
impl<'a> AddressPrefix<'a> {
    pub fn new(prefix_str: Cow<'a, str>, prefix_kind: Option<Prefix>) -> Self {
        AddressPrefix { prefix_str, prefix_kind }
    }
...
fn _convert_bits(data: impl Iterator<Item=u8>, from_bits: u32, to_bits: u32, pad: bool) -> Option<Vec<u8>> {
    let mut acc = 0;
    let mut bits = 0;
    let mut ret = Vec::new();
...
fn _to_cash_addr(prefix: &str, addr_type: AddressType, addr_bytes: &[u8]) -> String {
    ...
    String::from(prefix) + ":" + &_map_to_b32(payload.iter().cloned().chain(checksum.iter().cloned()))
}

fn _from_cash_addr<'a>(addr_string: &str, default_prefix: &'a str) -> std::result::Result<([u8; 20], AddressType, Cow<'a, str>), CashAddrError> {
    ...
    let (prefix, payload_base32): (Cow<'a, _>, _) = if let Some(pos) = addr_string.find(':') {
        ...
        (prefix[..prefix.len() - 1].to_string().into(), payload_base32)
    } else {
...
#[cfg(test)]
mod tests {
    use super::{Address, Prefix, AddressType, Hash160, Result};

    #[test]
    fn test_from_hash1() -> Result<()> {
        let addr = Address::from_hash(Prefix::BitcoinCash, AddressType::P2PKH, Hash160::new([0; 20]));
        assert_eq!(addr.cash_addr(), "bitcoincash:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqfnhks603");
        Ok(())
    }

    #[test]
    fn test_from_hash2() -> Result<()> {
        let addr = Address::from_hash(Prefix::SimpleLedger, AddressType::P2PKH, Hash160::new([0; 20]));
        assert_eq!(addr.cash_addr(), "simpleledger:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq9gud9630");
        Ok(())
    }

    #[test]
    fn test_from_hash3() -> Result<()> {
        let addr = Address::from_hash(Prefix::BitcoinCash, AddressType::P2SH, Hash160::new([0; 20]));
        assert_eq!(addr.cash_addr(), "bitcoincash:pqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq7k2ehe5v");
        Ok(())
    }
    ...
    fn test_from_hash4() -> Result<()> {
        ...
        assert_eq!(addr.cash_addr(), "redridinghood:pqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqxmg9w0gt");
        Ok(())
    }

    #[test]
    fn test_from_cash_addr1() -> Result<()> {
        let addr = Address::from_cash_addr("bitcoincash:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqfnhks603")?;
    ...
    fn test_from_cash_addr2() -> Result<()> {
        let addr = Address::from_cash_addr("simpleledger:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq9gud9630")?;
        assert_eq!(addr.addr_type(), AddressType::P2PKH);
        assert_eq!(addr.cash_addr(), "simpleledger:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq9gud9630");
        assert_eq!(addr.hash(), &Hash160::new([0; 20]));
    ...
    fn test_from_cash_addr3() -> Result<()> {
        let addr = Address::from_cash_addr("bitcoincash:pqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq7k2ehe5v")?;
